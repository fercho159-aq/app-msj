...
async function runMigration() {
    console.log('🚀 Agregando consultor Raúl Paredes...');
    ...
    try {
        const hashedPassword = await bcrypt.hash('RaulParedes2026.', 10);
        const rfc = 'CONS0005RP';
        const name = 'Raúl Paredes';
        const role = 'consultor';
        const avatarUrl = `https://ui-avatars.com/api/?name=Raul+Paredes`;
        ...
        console.log(`✅ Consultor Raúl Paredes creado exitosamente.`);
        console.log(`   RFC: ${rfc}`);
        console.log(`   Nombre: ${name}`);
        console.log(`   Rol: ${role}`);
        console.log(`   Contraseña: RaulParedes2026.`);
